...
class Solver(BaseSolver):
    def __init__(self, cfg):
        ...
        self.scheduler=torch.optim.lr_scheduler.MultiStepLR(self.optimizer,milestones=[self.cfg['schedule']['decay1'],self.cfg['schedule']['decay2'],self.cfg['schedule']['decay3']],gamma=self.cfg['schedule']['gamma'])
    ...
    def train(self): 
        with tqdm(total=len(self.train_loader), miniters=1,
            ...
            for iteration, batch in enumerate(self.train_loader, 1):
                ...
                # y = self.model(lms = bms_image, pan = pan_image)
                # # print(torch.min(abs(y-bms_image)))
                # # print(torch.max(abs(y-bms_image)))
                # loss = self.loss(y, ms_image) / (self.cfg['data']['batch_size'] * 2)  

                #
                AU, EU, hrms1, hrms2 = self.model(pan = pan_image, lms = lms_image)

                loss = self.l1_loss(hrms2, ms_image.detach(), 1)
                loss = loss + self.l1_loss(hrms1[0], ms_image.detach(), 1)*0.1 + self.l1_loss(hrms1[1], ms_image.detach(), 1)*0.1 #EU loss
                loss = loss + (self.U_loss(hrms1[0], ms_image.detach(),AU[0]) + self.U_loss(hrms1[1], ms_image.detach(),AU[1])) * 0.01  #AU loss
                loss  = loss + (self.l1_loss(hrms1[1], ms_image.detach(), 0.5*AU[0]+0.5*EU[0]) + self.l1_loss(hrms2, ms_image.detach(), 0.5*AU[1]+0.5*EU[1])) * 0.01 #focus loss
                ...
                loss.backward()
                # print("grad before clip:"+str(self.model.module.refine.conv_last.weight.grad))
            ...
            self.writer.add_image('image2', hrms2[0], self.epoch)
