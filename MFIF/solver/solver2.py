    ...
    def train(self): 
        with tqdm(total=len(self.train_loader), miniters=1,
            ...
            for iteration, batch in enumerate(self.train_loader, 1):
                ...
                y,y_I = self.model(lms_image, bms_image, pan_image)
                #print(torch.min(abs(y-bms_image)))
                #print(torch.max(abs(y-bms_image)))
                gt_I=torch.mean(ms_image,axis=1,keepdim=True)
                loss_I=self.loss(y_I, gt_I) / (self.cfg['data']['batch_size'] * 2)

                loss = self.loss(y, ms_image) / (self.cfg['data']['batch_size'] * 2)+0.1*loss_I
