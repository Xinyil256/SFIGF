...
import random
import imageio
from pytorch_ssim import SSIM as SSIM_pt

# from network import our_Net
# from skipnet_mcr_flops_2 import SkipNet as our_Net
from gfnet_v2 import GFNet as our_Net
...
def image_read(train_c_path, train_m_path, train_rgb_path):
    ...
    # for i in tqdm.tqdm(range(1)):
        ...
        train_c = os.path.join('/home/amax/Documents/yxxxl/oridata', train_c_path[i].split('/')[1],train_c_path[i].split('/')[2],train_c_path[i].split('/')[3] )
        color_raw = imageio.imread(train_c)
        inp_list.append(color_raw)

        train_m = os.path.join('/home/amax/Documents/yxxxl/oridata', train_m_path[i].split('/')[1],train_m_path[i].split('/')[2],train_m_path[i].split('/')[3] )
        mono_raw = imageio.imread(train_m)
        gt_m_list.append(mono_raw)
        
        train_rgb = os.path.join('/home/amax/Documents/yxxxl/oridata', train_rgb_path[i].split('/')[1],train_rgb_path[i].split('/')[2],train_rgb_path[i].split('/')[3] )
...
class load_data(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.training:
            ...
            """
            i = random.randint(0, (H - psz - 2) // 2) * 2
            j = random.randint(0, (W - psz - 2) // 2) * 2

            inp_raw = inp_raw_image[i:i + psz, j:j + psz]
            gt_m = gt_m_image[i:i + psz, j:j + psz]
            gt_rgb = gt_rgb_image[i:i + psz, j:j + psz, :]
...
if __name__ == '__main__':
    ...
    opt={'base_lr':3e-4}
    opt['batch_size'] = 5
    opt['iterations'] = 5000000

    metric_average_file = 'result_mcr_gfnet_v2_0825/metric_average.txt'
    # These are folders
    save_weights_file = 'result_mcr_gfnet_v2_0825/weights'
    save_images_file = 'result_mcr_gfnet_v2_0825/images'
    save_csv_file = 'result_mcr_gfnet_v2_0825/csv_files'
    ...
    while iter_num < opt['iterations']:
        for _, img in tqdm.tqdm(enumerate(dataloader_train)):
            ...
            loss_ssim = 1-SSIM_pt()(rgb_pred, rgb)
            # print(loss1.item(), loss2.item())
            loss = loss1 + 1*loss2 + 0.5*loss_ssim
            # print(loss1.item(), loss2.item(), loss_ssim.item())
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
            ...
            if iter_num % 100 == 0:
                ...
                ssim_rgb = 0
                # for idx in range(b):
                #     ssim_mono += SSIM(mono_raw[idx,...].detach().cpu().numpy().transpose( 1, 2, 0)*255,
                #                 np.clip(mono_pred[idx,...].detach().cpu().numpy().transpose( 1, 2, 0)*255, 0, 255))
                #     ssim_rgb += SSIM(rgb[idx,...].detach().cpu().numpy().transpose(1, 2, 0)*255,
                #                 np.clip(rgb_pred[idx,...].detach().cpu().numpy().transpose(1, 2, 0)*255, 0, 255), multichannel=True)

                print('\niter_num:%.0f, loss_mono:%.4f, loss_rgb:%.4f, PSNR_mono:%.4f, PSNR_rgb:%.4f, SSIM_mono:%.4f, SSIM_rgb:%.4f' % (
                iter_num, np.mean(loss1.detach().cpu().numpy()), np.mean(loss2.detach().cpu().numpy()), np.mean(psnr_mono), np.mean(psnr_rgb), ssim_mono/b, ssim_rgb/b))
            ...
            # run test during training, more CPU ram and GPU memory needed.
            # if iter_num % 40000 == 0:
            #     run_test(model, dataloader_test, iter_num, save_images_file, save_csv_file, metric_average_fil