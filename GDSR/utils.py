...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        # print(pred.shape)
        # rescale
        pred = pred * (data['max'].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) - data['min'].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)) + data['min'].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        gt = gt * (data['max'].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1) - data['min'].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)) + data['min'].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
    ...
    def train(self, train_loader, valid_loader, mid_loader=None, lu_loader=None, max_epochs=None):
        ...
        for epoch in range(self.epoch, max_epochs + 1):
            ...
            if self.epoch % self.eval_interval == 0:
                ...
                # self.evaluate_mid(mid_loader)
                # self.evaluate_lu(lu_loader)
    ...
    def test(self, loader, save_path=None):
        ...
        with torch.no_grad():
            for data in loader:
                ...
                gts = data['hr']
                ins = data['lr']
                gts = gts * (data['max'] - data['min']) + data['min']
                ins = ins * (data['max'] - data['min']) + data['min']
                ...
                for b in range(preds.shape[0]):
                    ...
                    lr = ins[b][0]
                    em = gt - pred
                    # em = (em - np.min(em)+0.001) / (np.max(em)-np.min(em)+0.001)
                    # image = (image - np.array([0.485, 0.456, 0.406]).reshape(3,1,1)) / np.array([0.229, 0.224, 0.225])
                    rgb = data['image'][b].permute(1,2,0).detach().cpu().numpy()
                    # em = em.clip(0.3,1)
                    # em = em * 12
                    # print('pred.shape', pred.shape, np.min(pred), np.max(pred))
                    # print('rgb.shape', rgb.shape, np.min(rgb), np.max(rgb))
                    rgb = rgb *  np.array([0.229, 0.224, 0.225]) +  np.array([0.485, 0.456, 0.406])
                    # (rgb - np.min(rgb)+0.001) / (np.max(rgb)-np.min(rgb)+0.001)
                    # save_gt_path = save_path+'_gt'
                    # save_em_path = save_path=
                
                    plt.imsave(os.path.join(save_path, f'{idx}.png'), pred, cmap='plasma')
                    plt.imsave(os.path.join(save_path, f'{idx}_gt.png'), gt, cmap='plasma')
                    plt.imsave(os.path.join(save_path, f'{idx}_lr.png'), lr, cmap='plasma')
                    # plt.imsave(os.path.join(save_path, f'{idx}_rgb.png'), rgb)
                    cv2.imwrite(os.path.join(save_path, f'{idx}_rgb.png'), rgb*255)

                    # cv2.imwrite(os.path.join(save_path, f'{idx}.png'), pred)
                    # cv2.imwrite(os.path.join(save_path, f'{idx}_gt.png'), gt)
                    # cv2.imwrite(os.path.join(save_path, f'{idx}_lr.png'), lr)
                    # plt.imsave(os.path.join(save_path, f'{idx}_em.png'), em, cmap='gray')
    ...
    def evaluate_mid(self, loader):
        self.log(f"++> Evaluate mid at epoch {self.epoch} ...")
        ...
        self.log(f"++> Evaluate mid epoch {self.epoch} Finished, average_loss={average_loss:.4f}")



    
    def evaluate_lu(self, loader):
        self.log(f"++> Evaluate lu at epoch {self.epoch} ...")
        ...
        self.log(f"++> Evaluate lu epoch {self.epoch} Finished, average_loss={average_loss:.4f}")
        ...
        with torch.no_grad():
            for data in loader:
                ...
                for b in range(preds.shape[0]):
                    ...
                    # em = em * 12
                    # print('pred.shape', pred.shape, np.min(pred), np.max(pred))
                    # print('gt.shape', gt.shape, np.min(gt), np.max(gt))
                    # save_gt_path = save_path+'_gt'
                    # save_em_path = save_path=
                    plt.imsave(os.path.join(save_path, f'{idx}.png'), pred, cmap='plasma')
                    plt.imsave(os.path.join(save_path, f'{idx}_gt.png'), gt, cmap='plasma')
